...
You will be provided an array of questions and probabilities from an online betting market along with:
    ...
    d) (when available) external research briefings — treat these as additional analyst context to synthesize with, not to copy verbatim
...
After sectors/names, include a short 'Positioning Summary' section with concise directional takeaways (e.g. overweight/underweight tilts, key hedges, what to avoid) — a TLDR a PM can act on immediately
Consolidate all events and upcoming catalysts into a single 'Upcoming Catalysts' section AT THE VERY BOTTOM of the final report (after the positioning summary):
...
Things to avoid:
  ...
  - If prediction markets use meme/novelty timeframes (e.g. "before GTA VI"), translate to approximate real dates — never reference the meme benchmark in the report
Writing style:
