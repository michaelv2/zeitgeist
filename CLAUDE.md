...
- `validate_verifier_ledger.py`: false-positive property test for the ledger-aware verifier — feeds a crafted memo with one manufactured inflection (tell untripped) and one genuine, tell-warranted inflection; checks the verifier flags the former and HOLDS the latter (the false-positive direction that would undo the ledger's value)
...
4. **Verifier + Revise** (`verifier_agent` → `revise_agent`) — a flag-then-revise quality gate, gated by `ENABLE_VERIFIER`
   - **Verifier** (`VERIFIER_MODEL` = Opus 4.8, adaptive thinking): re-reads the finished memo adversarially for OVER-reach — overstatement, self-contradiction, ungrounded claims, cherry-picked framing, and stale/ungrounded catalysts. Has the same bounded FRED tool to ground numeric claims, and is given the Upcoming Catalysts list to flag timing pegs that aren't actually upcoming. When the ledger is enabled it also receives the **prior themes** and runs a ledger check — flagging an inflection the evidence doesn't carry (or the inertia case: a faded theme carried forward as `intact`), held to the same HIGH bar. Returns structured `Finding`s (or none)
